...
def test(args, model=None, savedir=""):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            _, ES_logits, SE_logits = model(**inputs)

        token_type_ids = batch[2]
        for ES_logit, SE_logit, token_type_id in zip(ES_logits, SE_logits, token_type_ids):
            ES_logit = ES_logit.detach().cpu().tolist()
            SE_logit = SE_logit.detach().cpu().tolist()
            ...
            select = perResults(ES_logit, SE_logit, question_len, passage_len)
